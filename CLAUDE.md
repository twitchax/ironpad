...
- **Saved outputs (PRD-0056)**: 43 of the 45 notebooks carry `saved_output` per runnable cell, captured by `tools/capture-outputs.mjs` (seeds each notebook as a scratch `/local` copy, runs cells one at a time, and drives the app's own Download flow — the production capture path). Re-run it after changing a notebook's code so its rendered outputs stay honest. `borrows` and `dynosaur` capture nothing on purpose: each opens with a deliberate compile-fail teaching cell, and running any later cell cascades that failure, aborting the queue — the same wall a reader hits pressing Run today.
