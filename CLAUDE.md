...
**Mutable shares (PRD-0054: server-authoritative draft/published)**. ONE address per published notebook: `/mutable/{id}` is the view-only reader of PUBLISHED for everyone and the live editor over the server-side DRAFT for the owner (auto-swapped on hydrate; `?view=reader` pins the reader; SSR always renders the reader so crawlers/readers can never see draft content). The `mutable_share` record carries `notebook_json` (published; `option` since PRD-0064, `None` = never published), `draft_json` (option; `None` = clean), and the manifest. The editor gains a storage seam (`NotebookState.server_draft_share`): Local mode persists to IndexedDB exactly as before; ServerDraft mode debounce-autosaves the draft (1.5s, epoch-coalesced, retry-on-failure, `save_mutable_draft`) with a "Saving draft…" indicator. The toolbar **Push button** is the one editorial act: "Publish" while no published copy exists (PRD-0064), grayed "Published ✓" when clean, "⬆ Push" when the draft differs; Push flushes + durable-saves the draft, then `push_mutable(id, tags)` promotes it server-side (uploads NOTHING) and snapshots blobs from the draft at that moment. Push ABORTS loudly if the durable save fails (`persist_notebook_durable` returns success; a `draft_save_inflight` counter drains in-wire autosaves so the durable write lands last before the promote). Draft autosaves count toward `MAX_TOTAL_MUTABLE_BYTES`. The share/publish/push/discard/unpublish/download workflows live in `notebook_editor/sharing.rs` with ONE flush-before-serialize helper (Unpublish once skipped the flush and lost the last debounce-window of typing before deleting the only other copy; Download serializes from the live model — a published notebook has no IndexedDB record). Menu: "View as Reader", confirm-gated "Discard Draft" (clears `draft_json`), Unpublish (PRD-0064: clears the published copy IN PLACE, keeping the notebook in the account at the same URL; it no longer saves to IndexedDB and navigates to `/local/{uuid}`), Delete (removes the notebook from the account entirely). Share Mutable uploads, DELETES the local copy, and hard-navigates to `/mutable/{id}`. There is NO local mutable store anymore (storage.js DB_VERSION 5 deletes it): home lists three sources with zero reconciliation (Local = IndexedDB, Account = `list_mutable_shares()` by session, Public = server scan; the chips were renamed from Private/Published in PRD-0064), and the divergence banner, Pull Latest, clone-to-local, and the share binding are all deleted. Concurrency is last-write-wins on the draft at autosave granularity (documented, not solved). Server fns: `save_notebook_to_account` (PRD-0064; the ONE upload path), `create_mutable_share` (that plus a publish), `save_mutable_draft`, `get_mutable_for_edit` (owner-gated; draft + dirty + published), `push_mutable` (promote; `Ok(false)` = already clean, which a first publish never is), `discard_mutable_draft`, `get_mutable_notebook` (readers; published + attribution + is_owner), `get_mutable_manifest`, `unpublish_mutable_share` (PRD-0064), `delete_mutable_share`, `list_mutable_shares`. Published notebooks are online-only to edit; private notebooks stay local-first.

**Account notebooks (PRD-0064: publishing is a flag on a row, not a storage class)**: **Save to Account** uploads the notebook, DELETES the local IndexedDB record, and hard-navigates to `/mutable/{id}` — move, never copy, the same discipline Share Mutable already followed, because two unreconciled copies of one notebook is precisely what PRD-0054 removed. There is no second table, route or editor mode: an account notebook IS a `mutable_share` whose `notebook_json` is `None`, with its content in `draft_json`, which is where the ServerDraft editor already writes. `content = draft_json.or(notebook_json)` (`db::resolve_content`, the one place that rule is written). The state machine is `(notebook_json, draft_json)`: `(None, Some)` = in account, reader 404, button **Publish**; `(Some, None)` = published and clean, grayed **Published**; `(Some, Some)` = published and dirty, **Push**. An unpublished notebook is permanently dirty by construction, so `dirty` alone cannot tell a first publish from a republish — `ShareEditRow.published` is a separate flag and `PublishButton::from_flags(published, dirty)` (`notebook_editor/state.rs`) is the one derivation. **Both slots `None` is corruption, not a state**: `discard_draft` is guarded on a published copy existing (discarding an unpublished notebook's draft would delete its only content), `unpublish_share` folds published into the empty draft slot in the same UPDATE that clears it, and `list_shares_owned_by` WARNs and drops such a row rather than failing the call, so one bad notebook cannot hide every other one. **Unpublish is the one behavior this PRD takes away**: it clears `notebook_json`/`manifest_json`/`pushed_at` in place and the notebook stays in the account at the same URL (publishing again is a Push). The old flow wrote the notebook into IndexedDB, deleted the share, and navigated to `/local/{uuid}`, and for one moment that local write was the ONLY surviving copy — which is why it carried a load-bearing flush and a confirm about losing it. Download `.ironpad` is now the only way back to a local file; **Delete** removes the notebook from the account. Visibility needs no new rule: `get_mutable_notebook_core` returns `None` with no published copy, the same shape as PRD-0061's private denial, so the reader page, `/embed/mutable`, the OG card and oEmbed 404 through the existing gate — **except the manifest**, which has its own core (`mutable_manifest_access_core`) reading `manifest_json` off the row and funnels through NEITHER `get_mutable_notebook_core` NOR `mutable_access_core`, so it carries an explicit `notebook_json.is_none()` arm; the manifest is the hash list and `/share-blobs/` is ungated on the theory that hashes are only knowable FROM a manifest, so handing one out hands out the blobs. `crates/ironpad-server/tests/unpublished_notebooks.rs` asserts every anonymous surface over RAW responses (status AND no notebook text anywhere in the body) for both denied identities, anonymous and signed-in stranger; the save/publish/unpublish/delete lifecycle is `tests/e2e/account-notebooks.spec.ts`. Schema: `notebook_json` and `pushed_at` widened to `option<string>` via **`DEFINE FIELD OVERWRITE`**; every other field keeps `IF NOT EXISTS`, which silently declines to redefine an existing field (see the stamp — this is the trap CI structurally cannot see). Quotas: `admit_mutable_write` is the ONE admission point for both `MAX_TOTAL_MUTABLE_BYTES` (512 MiB, instance-wide) and `MAX_PER_USER_MUTABLE_BYTES` (32 MiB, summed over the shares carrying one user's OWNER grant); storing notebooks is the point of an account, so a global cap alone would let one account take the instance. Home chips are Local / Account / Public (`FilterMode::Private`→`Local`, `Mutable`→`Account`), unpublished rows list with a muted lock badge, and ordering is `last_activity()` = `pushed_at` else `created_at` (ordering on `pushed_at` alone sinks every unpublished notebook below every published one). `ShareAccessSection` renders only when published: a private toggle over something nobody can reach changes nothing.
...
- `/mutable/{id}` — MutableNotebookPage (PRD-0054: reader of published for everyone; the owner's draft editor on hydrate; `?view=reader` pins the reader). PRD-0064: with no published copy the reader 404s for everyone but the owner, who still edits at this same URL
...
Current server functions: `compile_cell`, `check_cell` (live check-on-type, PRD-0045), `list_public_notebooks`, `get_public_notebook`, `share_notebook`, `get_shared_notebook`, `get_shared_manifest` (blob-snapshot sidecar, PRD-0047), `get_toolchain_fingerprint` (client cache keys, PRD-0047), the mutable-share set (PRD-0054, session-gated writes): `save_notebook_to_account` (PRD-0064), `create_mutable_share`, `save_mutable_draft`, `get_mutable_for_edit`, `push_mutable` (draft promote), `discard_mutable_draft`, `get_mutable_notebook`, `get_mutable_manifest`, `unpublish_mutable_share` (PRD-0064), `delete_mutable_share`, `list_mutable_shares`, and `get_auth_info` (header auth surface, PRD-0053).
...
**Toolchains**: cell builds pin their toolchain explicitly via `cell_toolchain` (in `compiler/build.rs`), never the host default (dev boxes, CI, and the deploy image all differ, and that divergence once shipped cells that compiled locally and failed on prod). Since PRD-0067 there are **two pins, and the target alone picks between them**:
- **`CELL_TOOLCHAIN`** (`nightly-2026-05-19`) — every cell whatever its features. Defined ungated in `crates/ironpad-app/src/lib.rs`, displayed in the footer, and it is also `rust-toolchain.toml`'s channel, so the workspace builds on the compiler cells are judged by.
- **`BROWSERPOD_TOOLCHAIN`** (`browserpod-3.0.0`) — Linux cells only (PRD-0066), the sole toolchain that can build `wasm32-browserpod-linux-musl`.

This was FOUR nightlies (~2.9 GB of image) until PRD-0067 collapsed it to one (~900 MB). **The date is forced from both sides.** Autodiff is the ceiling: July 2026 nightlies fail with `internal compiler error: incorrect autodiff typetree handling for slice`, and since `-Zautodiff=Enable` applies to the whole crate graph the ICE lands on a transitive dependency (`icu_normalizer`), so no cell-side workaround exists. BrowserPod is the floor: its pack ships 22 prebuilt rlibs ABI-tied to this rustc and its `libexec/{rustc,cargo}` are symlinks into it, so the nightly is in the image regardless. One date sits between them, and it was already paid for. **`CELL_TOOLCHAIN` must therefore equal `BROWSERPOD_NIGHTLY` in `docker/browserpod.env`**: drift breaks nothing and silently costs a whole toolchain, so `browserpod_pin_matches_cell_toolchain` fails the build instead. Taking a BrowserPod pack upgrade now means re-verifying autodiff and rayon on its nightly, which is the accepted price of the collapse.

**The atomics pin was the interesting one, and it nearly caused a bad ship.** Its comment blamed wasm-bindgen-rayon's `atomics` target-feature guard "breaking on newer nightlies". That mechanism is FALSE: rayon compiles clean five months past the pin, `libwasm_bindgen_rayon.rlib` lands in the graph, the `compile_error!` never fires, and the linked blob carries every TLS export. On the strength of disproving the stated mechanism the pin was declared stale, and that conclusion was reported before it was true. **`build_micro_crate` does not stop at `cargo build`**. It runs `wasm-bindgen` afterward, and THAT fails on every newer nightly with `failed to prepare module for threading / failed to find __heap_base for injecting thread id`. Isolating the variable (two CLI versions over ONE identical blob) put the cause where it actually was: **wasm-bindgen 0.2.114 could not read a modern nightly's output; 0.2.127 can.** The pin was holding rustc back to accommodate a CLI thirteen patch versions stale, so PRD-0067 upgraded the CLI + crate family (js-sys/web-sys 0.3.91 -> 0.3.104; leptos needed nothing) and the pin dissolved. `rust-toolchain.toml`'s `thaw` justification, by contrast, was simply dead, and moving it unmasked two real clippy findings the older compiler had been hiding. **The method note worth keeping: `cargo check` cannot see a codegen bug (Enzyme runs during LTO) and `cargo build` cannot see a post-processing bug.** Same error one level out, made in the same session that warned about the first one. Only the full pipeline discriminates, which is why `compile_rayon_cell_links_a_shared_memory_module` now BUILDS a rayon cell and asserts the POST-wasm-bindgen surface (`initThreadPool`, `wbg_rayon_start_worker`, `__wbindgen_thread_destroy`); the raw `__wasm_init_tls`/`__tls_size`/`__tls_align` exports are CONSUMED by the threading transform, so asserting them fails against a correct module. Nothing built a rayon cell before this; the only coverage was `all_public_notebook_cells_compile`, which calls `check_micro_crate`, which is exactly how a load-bearing constraint went unmeasured for the whole life of the feature. Also measure the artifact that ships: toolchain sizes taken locally are off by 3x because local installs are default-profile while the image is `--profile minimal` (758 MB of the old 2.1 GB atomics pin was docs).

The feature handling did NOT collapse with the toolchains: atomics cells still get their target features, link args and `-Zbuild-std`; autodiff cells still get `-Zautodiff=Enable` and a forced fat-LTO profile. Cargo fingerprints those apart by RUSTFLAGS and profile exactly as it used to by rustc version, so they stay warm side by side, and the atomics warmup survives as its own image layer (a `-Zbuild-std` std rebuilt with `+atomics` is its own sysroot). The cache fingerprint describes `CELL_TOOLCHAIN`'s rustc (plus the wasm-bindgen CLI), so bumping it invalidates all blobs automatically, and with one pin inside the fingerprint, **the old "bumping AUTODIFF/ATOMICS needs a manual `CACHE_EPOCH` bump" trap is gone by construction.** **In the deploy image the version string is BAKED at build time** (`docker/Dockerfile` writes raw `--version` output to `/app/toolchain-versions`; `IRONPAD_TOOLCHAIN_VERSIONS_FILE` points the server at it) and read back through the same parsers used on live output, so the fingerprint is byte-identical to the probed one and cached blobs survive. Probing it at runtime cost a cold `rustc --version` that demand-pages ~350 MB of libLLVM + librustc_driver: **5.9s of a 7.2s Fly cold start**, for a string fixed when the image was built. Anything unset/missing/malformed falls back to probing, so dev boxes and CI are unchanged; the boot log's `toolchain fingerprint source=` field (`baked` vs `probed`) says which path a given deploy took. Server startup no longer awaits that warm-up either — `prewarm` is detached so the listener binds first. `docker/Dockerfile` and `.github/workflows/build.yml` must install both pins (the image drops stable entirely). Autodiff cells carry distinct RUSTFLAGS + a fat-LTO profile, so they never shared the Docker default-target warmup; they're warmed post-deploy by the runtime cache warmer.
...
**Last Updated**: 2026-08-17 — Linux cells finally got their public notebook, and the PRD that shipped them was wrong in three places. **The notebook.** `public/notebooks/linux-cells.ironpad` (10 cells, 5 Linux) is the 47th public notebook and the first carrying a `CellType::Linux` cell: a real process reporting its pid and procfs, a file written by one process and read back by another over the pod's shared filesystem, a `cut | sort | uniq -c` pipeline of three chained children with no bytes passing back through the program, and a thread fan-out. **Its stated premise was false and measuring corrected it.** T-013's note read "sched_getaffinity is imported, so available_parallelism() should return something real"; it returns **1**, and `nproc` agrees, so the notebook hard-codes its worker count and says why. **The threads are real anyway, and the first measurement said the opposite.** A probe counting primes below 200,000 gave 19ms serial against 38ms on two threads and 20ms on four, which reads as "no parallelism" — and the SAME probe measured **20ms to spawn four empty threads**, so the whole workload fit inside the spawn cost. Re-run at 6,000,000 the answer inverts: **2,045ms on one thread, 194ms across sixteen, 10.5x**. The conclusion was formed off the first run and stated before the second existed; a speedup test whose serial arm finishes in 19ms is not a speedup test, and the cell now warms the Worker pool before timing because measured cold the two-thread row comes out SLOWER than serial. Also measured: fourteen back-to-back `Command::output()` calls, the fourteenth failing `EAGAIN` from an `unwrap` inside `std::sys::process` (each child is a Web Worker; exit 137). The exact ceiling is NOT established, one observation and no sweep, so the notebook reports what was seen and names no mechanism. **Three coverage holes, none of which any gate could see.** `all_public_notebook_cells_compile` skipped every `cell_type != Code`, so the first Linux-cell notebook would have shipped with ZERO compile coverage while the suite stayed green; it now routes them through `CellTarget::Linux` and exempts them from the `cellN`/`last` skip, since a Linux cell has no typed piping and `last` in one is an ordinary identifier. **`cargo make test-linux-cells` could not have run at all**: `playwright.config.ts` hard-coded the deliberately fake key, so the opt-in project would have exercised the no-key failure path instead of booting a pod — which is exactly why `tests/e2e/linux-pod/` sat empty from the day it was created and uat-002/003 were never verifiable. The key now comes from `.hidden/dev.env` and the task refuses to run against the fake one. And uat-006 had **no test** despite being a licensing term rather than a feature; it now sits in the DEFAULT gate (attribution renders without a pod, so pointing it at a metered run was wrong) and asserts the mark occupies real space and its asset resolves, because `toBeVisible` passes on a zero-area span whose mask 404s. **The frontmatter was stale in a way CI structurally cannot catch**: T-003 through T-012 and T-014 all read `todo` against a tree where the feature had shipped in three commits, every UAT read `unverified`, and `PROTOCOL_VERSION` was never bumped for the two new `CellType` variants that ride the wire on every `IronpadCell` (now **7**; old peers do not break on an unknown tag, which is precisely why the number is the only signal anything changed). The pod-booting specs live in `tests/e2e/linux-pod/notebook.spec.ts` and drive the SHIPPED notebook rather than cells of their own, because Linux cells carry no `saved_output` and are therefore invisible to the `capture-outputs` freshness gate that covers every other notebook. PREVIOUSLY — Four cell toolchains collapsed onto one, and the pin that looked stalest turned out to be load-bearing. **The shape.** Cell compilation ran on four pinned nightlies (2,938,996,672 bytes of image, measured by installing each set into a `rust:1.93.0-slim` container exactly as the Dockerfile does); it now runs on `nightly-2026-05-19` alone, at 795,512,039 bytes. **2.14 GB, 73%.** The date is forced from both sides: autodiff is the ceiling (July 2026 nightlies ICE with `incorrect autodiff typetree handling for slice`, and since `-Zautodiff=Enable` applies to the whole crate graph the ICE lands on a transitive dependency, `icu_normalizer`, so no cell-side workaround exists) and BrowserPod is the floor (its pack ships 22 prebuilt rlibs ABI-tied to that rustc and its `libexec/{rustc,cargo}` are symlinks INTO it, so the nightly was in the image regardless). One date sits between them and it was already paid for: the collapsed toolchain is 772 MB against the 591 MB the pack was already pulling, so serving every other cell from it costs **181 MB of components**. That equality is now enforced: `browserpod_pin_matches_cell_toolchain` fails the build if `CELL_TOOLCHAIN` and `BROWSERPOD_NIGHTLY` drift, because drift breaks nothing and silently regrows the image. **The correction that matters, reported before it was true.** `ATOMICS_TOOLCHAIN`'s comment blamed wasm-bindgen-rayon's `atomics` guard "breaking on newer nightlies". That mechanism is FALSE and disproving it was easy: rayon compiles clean five months past the pin, `libwasm_bindgen_rayon.rlib` lands in the graph so the `compile_error!` was evaluated and never fired, and the linked blob carries every TLS export at 1,018,807 bytes. The pin was declared stale on that basis. **`build_micro_crate` does not stop at `cargo build`**. It runs `wasm-bindgen`, and THAT fails on every newer nightly with `failed to prepare module for threading / failed to find __heap_base for injecting thread id`. Isolating the variable (two CLI versions over ONE identical blob) put the cause where it actually was: **wasm-bindgen 0.2.114 cannot read a modern nightly's output and 0.2.127 can**, so the pin was holding rustc back to accommodate a CLI thirteen patch versions stale. Upgrading the CLI + crate family (js-sys/web-sys 0.3.91 -> 0.3.104; leptos needed nothing; the pin is duplicated in `Makefile.toml`, `build.yml` and the Dockerfile and all three must move together) dissolved the pin and fixed the cause instead of the symptom. **The method note: `cargo check` cannot see a codegen bug (Enzyme runs during LTO) and `cargo build` cannot see a post-processing bug, and the second error was made in the same session that warned about the first.** Only the full pipeline discriminates, so `compile_rayon_cell_links_a_shared_memory_module` now BUILDS a rayon cell and asserts the POST-wasm-bindgen surface (`initThreadPool`, `wbg_rayon_start_worker`, `__wbindgen_thread_destroy`); the raw `__wasm_init_tls`/`__tls_size`/`__tls_align` exports are CONSUMED by the threading transform, so a first draft asserting THEM failed against a correct module. Nothing built a rayon cell before this; the only coverage was `all_public_notebook_cells_compile`, which calls `check_micro_crate`, and that is exactly how a load-bearing constraint went unmeasured for the whole life of the feature. `rust-toolchain.toml`'s pin was the genuinely dead one: held for `thaw`, deleted in v0.12.13 and absent from `Cargo.lock`, and moving it immediately unmasked two real clippy findings (`explicit_counter_loop`, `unnecessary_trailing_comma`) plus nine more across the workspace that the older compiler had been hiding. Two more traps: **a first e2e run failed 11 specs and that was COLD CACHE, not breakage** (changing `CELL_TOOLCHAIN` changes the fingerprint so every cell recompiles; a spec comment already said it assumes a warm cache; the re-run passed 166 of 166 in half the wall time), and **toolchain sizes measured locally are off by 3x** because local installs are default-profile while the image is `--profile minimal`: 758 MB of the old 2.1 GB atomics pin was documentation the image never installs, and an early "3.7 GB saved" estimate came from exactly that error. With one pin and it inside the fingerprint, the old "bumping AUTODIFF/ATOMICS needs a manual `CACHE_EPOCH` bump" trap is gone by construction. PREVIOUSLY — Concurrent writes to one record used to fail, and surrealdb moved to 3.2.4. **The bug.** `SurrealKV` is optimistically concurrent: two writers touching ONE record race, one commits, and the losers get a conflict whose text says, in the engine's own words, "This transaction can be retried". Nothing retried it. Measured with 8 parallel writers, `save_draft` landed 1 of 8 — and PRD-0064 had just made the server draft the PRIMARY storage path, so two browser tabs on one account notebook was enough, with the loser's autosave surfacing as "Draft not saved; retrying" forever. Over HTTP, 8 concurrent sign-ins as one user returned seven 500s, against 8 of 8 successful for distinct users as the control. `with_conflict_retry` now wraps the 14 methods that write `mutable_share` or `user`: five bounded attempts, jittered backoff, retrying ONLY a recognised conflict and passing every other error through. Bounded because a conflict surviving five attempts is not contention any more, and swallowing it trades a fast error for a hung request. `promote_draft` and `update_mutable_share` clone `manifest_json` per attempt, because a retry closure runs its body more than once — the compiler caught that, which is the argument for wrapping whole bodies rather than editing each query. **`#[tokio::test]` defaults to a CURRENT-THREAD runtime**, where these tasks interleave at await points instead of running in parallel: the first probe reported 8 of 8 succeeding against the UNFIXED code and nearly closed the investigation. The regression test pins `flavor = "multi_thread"`, and its negative control disables ONLY the retry arm while leaving all 14 wrappers in place, so it distinguishes "the retry fixes it" from "the refactor happened to fix it". **The upgrade is FORWARD ONLY.** A database written by 3.2.4 cannot be opened by 3.0.5 (`Invalid revision \`2\` for type \`Node\``), found when a dev database upgraded during testing crashed the old binary on boot; the reverse direction was verified by writing with 3.0.5 and reading with 3.2.4 through the real accessors. A pre-upgrade copy of prod lives in `.hidden/backups`, and restoring it means pinning the old binary too. 3.2.4 is FASTER (reads 2.8x, writes ~1.3x, measured directly at 200 iterations); an earlier "15x slower" reading came from a Playwright run containing two failures, whose 15s timeouts were being counted as engine time. It does NOT fix the WAL: `max_memtable_size` became configurable in 3.1.5, but `ConfigMap::from_env` is never called in surrealdb-core and the embedded path (`Datastore::new` -> `Builder::build_with_path`) exposes no config seam — proven by setting the env var to 1MiB, writing 20,000 records, and finding `sstables=0` with a byte-identical WAL. **Method note that cost the most this session:** five confident claims were wrong (a CI hydrate hole that did not exist, a reachable memtable knob, an upgrade-caused concurrency regression, a 15x slowdown, and earlier the WAL boot-write theory), and every one came from reading a signal that looked decisive and inferring a mechanism rather than isolating a variable. The claims that survived are the ones built with a control. Measure BEFORE forming the opinion, because the opinion decides which evidence looks sufficient. PREVIOUSLY — Account notebooks (PRD-0064): signing in now buys storage, not only publishing. **The shape.** Save to Account is Share Mutable minus the promote (upload, delete the local IndexedDB record, hard-navigate), and an account notebook is a `mutable_share` with `notebook_json` NONE whose content lives in the draft slot the editor already writes to — so publishing is `push_mutable` unchanged and nothing ever moves between storage classes. `create_mutable_share` became a wrapper over `save_notebook_to_account` + promote, because two upload paths meant two places to validate JSON, meter bytes and mint an id, and only one of them would ever get the next fix. **The migration cannot fail in CI, which is the whole problem.** Every DEFINE FIELD in `define_schema` carries `IF NOT EXISTS`, and SurrealDB silently declines to redefine a field that already exists; dev boxes and CI open a FRESH database every run, so widening `notebook_json` to `option<string>` would look applied everywhere except the one database that predates it, surfacing for the first time in production as a rejected write on the first Save to Account after deploy. `DEFINE FIELD OVERWRITE` on exactly those two fields, everything else untouched. The regression test BUILDS a pre-0064 database with the old DDL, inserts a published row, then runs `define_schema` and asserts both that the old row still reads and that a NONE write is now accepted, plus that the OVERWRITE is re-runnable since the DDL runs on every boot. A test that opens a fresh database proves nothing. **"Visibility is free" was false in exactly one place.** The design claim is that every reader-facing surface funnels through `get_mutable_notebook_core`/`mutable_access_core`, so an unpublished notebook 404s with no new rule. True for the reader page, `/embed/mutable`, the OG card and oEmbed. NOT true for `get_mutable_manifest`, which has its own core reading `manifest_json` off the row; it now carries an explicit `notebook_json.is_none()` arm, and that matters because `/share-blobs/` is deliberately ungated on the theory that hashes are only knowable FROM a manifest — hand out the manifest and the blobs go with it. **Both denied identities, against raw bodies, with a positive control.** PRD-0050 and PRD-0063 each shipped a status that was correct in the browser and wrong on the wire, and PRD-0063's raced on identity alone (anonymous 404, signed-in non-admin 200) because only the signed-in path needs a session lookup before the shell flushes. `crates/ironpad-server/tests/unpublished_notebooks.rs` drives the real router and asserts the status AND the absence of every distinctive notebook string from the body, for anonymous and for a signed-in stranger, then publishes and asserts the same surfaces come alive: without that half, every assertion above it would also pass against a router serving nothing. **Three smaller traps.** `mutable_share_exists` SELECTed `pushed_at` — a CONTENT field an account notebook does not have — in the one check deciding whether a freshly minted id is free, so an unpublished row would have read as a deserialization failure there; it reads `record::id(id)` now. Both slots NONE is corruption rather than a state, so `discard_draft` is guarded on a published copy existing (it would otherwise delete the only content of an unpublished notebook), `unpublish_share` folds published into the empty draft slot, and the owner listing warns-and-drops such a row rather than failing the whole call. And SurrealDB evaluates every right-hand side against the PRE-update document, so `draft_json = draft_json ?? notebook_json` sees the published content whether it is written above or below `notebook_json = NONE` — measured by swapping them and watching the test stay green, which means the test gates that the move exists, not the order. **The one behavior taken away**: Unpublish no longer hands back a `/local` notebook. It clears the published copy in place, the notebook stays in the account at the same URL, and publishing again is a Push; Download `.ironpad` is the way to a local file and Delete is the way out of the account. That REMOVES the moment where an IndexedDB write was the only surviving copy of the notebook, rather than guarding it with a load-bearing flush and a confirm. Quotas: `MAX_PER_USER_MUTABLE_BYTES` (32 MiB, summed over one user's OWNER grants) beside the 512 MiB instance cap, both enforced at ONE admission point (`admit_mutable_write`) with a deliberate minor TOCTOU — these bound steady-state storage, they are not hard quotas. PREVIOUSLY — Admin operations panel (PRD-0063) and per-route script loading. **The panel.** `/admin` is gated by `IRONPAD_ADMIN_LOGIN` plus a real signed-in session; unset means the surface DOES NOT EXIST (contributors and CI get that by default), and denial is an ordinary 404 rather than a 403, which deliberately inverts PRD-0061's explicit-denial call because nobody arrives at `/admin` by invitation. `auth::admin_user` is the ONE gate, and `admin_fns_are_all_gated` SCANS `server_fns.rs` asserting every `pub async fn admin_*` calls it: a runtime test only covers functions someone remembered to list, and the failure that matters is an admin fn written next year. It was verified by watching it fail on an injected ungated fn. Identity pins to `github_id` on first match (`meta:admin_github_id`), because db.rs already notes logins can be RENAMED and GitHub frees the handle for anyone to claim. **Three bugs worth remembering, each hidden by the one before it.** `SELECT VALUE count() ... GROUP ALL` returns `{count: N}` and needs SurrealDB's `SurrealValue` derive, not serde. That was invisible because the page rendered "Page not found." for ANY `Err`, so a broken database looked exactly like "you are not the admin" to the only person who could fix it; admin fns now return `Ok(None)` for denial and `Err` for real failure. And the 404 RACED: anonymous visitors got 404 while signed-in non-admins got **200**, because under out-of-order streaming the status commits when the shell flushes and only the signed-in path needs a session lookup first. `SsrMode::Async` fixes it, the same lever PRD-0050 used. Only an e2e can catch that one: it is invisible in the DOM. **Cache tiers** are a `CacheTier` enum in ironpad-common (a server fn taking a `&str` and joining it onto the cache path is a directory traversal); `blobs` carries `valve_may_clear() == false` so the unattended boot valve can never take compiled output, while an admin can. **Per-route scripts.** The shell loads KaTeX and Prism only on routes that render markdown, taking the home page from 177,291 to 57,633 bytes brotli (67%). Only those two are route-dependent, and only because neither has a synchronous Rust caller: both install a MutationObserver and sweep the existing DOM, so arriving late is normal. Monaco, sortable and the executor still load everywhere because Rust reaches for their globals from mount effects that cannot await (the Monaco effect guards double-init synchronously, which an await would make racy). The AMD hazard needed REAL handling: tag order used to keep KaTeX and Prism ahead of Monaco's `define`, and route-dependent loading destroys that accident. A first attempt masked `define` when `loadEntry` was CALLED, which is before Monaco's loader executes, so it never fired and `window.katex` came back undefined with 25 math spans sitting unrendered. The loader now schedules UMD entries first and masks `define.amd` rather than `define` (removing `define` would break any Monaco module executing in that window). **Method notes.** A background command's exit code is the LAST statement's, so `cmd > log; echo $?` inside a compound reports the echo: two gates were announced green while red this session. Capture and check the exit explicitly. And a test that passes has proven nothing until you have watched it fail: `robots_hides_only_the_embed_surface` kept passing when `/admin` was added and its name stopped being true, and a first-paint test comparing FCP against the last script's `responseEnd` inverted harmlessly on a fast run (it now injects a 1500ms delay and asserts paint lands first). PREVIOUSLY — First-paint pass on the shell. Twelve classic `<script>` tags sat in `<head>` with NO `defer` and no `async`, so the parser stopped at each one before drawing anything. Measured on prod against a **warm** server with a cold cache: `ttfb 116ms, fcp 378ms, 12 scripts / 164KB blocking for 223ms` — **223ms of the 262ms between first byte and first paint**, spent on KaTeX (94KB of the 164KB), Prism, Monaco and sortable, on a home page that is a list of notebook cards and uses NONE of them. Two constraints made this more than a one-word fix. (1) `defer`, never `async`: deferred classic scripts still execute in DOCUMENT ORDER, which is exactly what the KaTeX/Prism-before-Monaco ordering needs (both ship UMD bundles that register as anonymous modules and never assign their globals if AMD's `define` already exists). The old comment cited "no defer" as the mechanism for that ordering — the wrong lever for a real constraint. (2) Deferring ALONE would have been a bug, because `/pkg/` is served `immutable` while these are served `no-cache`, so on a repeat visit the wasm is ready from cache with no network at all while `storage.js` is still revalidating. Two facts were CHECKED rather than assumed: leptos 0.8 has no `DOMContentLoaded` handling anywhere in its source (`hydrate_body` mounts the moment it is called), and wasm-bindgen's glue resolves a `js_namespace` at CALL time (`IronpadStorage.foo()` as a bare global), so hydrating early does not fail at load — it fails later and less legibly, the first time an effect reaches for a global whose script has not run. So `hydrate_body_when_shell_ready` waits for `DOMContentLoaded`, which deferred scripts are guaranteed to run before, and the shell CAPTURES that event in an inline promise because it is not queryable after the fact: `readyState` turns `interactive` BEFORE deferred scripts execute and stays that way after `DOMContentLoaded` fires, so it cannot tell shell-pending from shell-ready. Verified by delaying every shell script 1000ms on ONE build with only the attribute varying between runs: FCP moved **-9ms deferred vs +1054ms blocking**, and both runs still hydrated (the half a paint metric cannot see). `tests/e2e/first-paint.spec.ts` asserts all four properties. Bytes are UNCHANGED; loading these per-route instead of on every route is the larger win and a separate change. Method note: the first prod measurement read 2.66s FCP and was nearly blamed on the scripts — TTFB alone was 2,402ms of it, because the machine was cold-waking. The scripts were 164ms. **Warm the server before attributing anything**, and separate TTFB from client time before naming a cause. Also measured while in there, not yet acted on: the SurrealKV WAL has grown to **64,227 batches / 4.82MB / 69ms replay** (was 25,424 / 2.2MB), and the manifest reads `tables=0` — the memtable has NEVER flushed to an SSTable, so nothing is ever compacted away and the single WAL segment cannot rotate. The volume is at 4.0G of 9.9G (`targets` 3.2G, `cargo-home` 558M, `blobs` 166M/1,915 entries), and `cache_valve.rs` wipes rebuildable tiers at 80%. PREVIOUSLY — Mobile/touch pass, found by DRIVING the deployed site at 390/600/768px rather than by reading the stylesheet (the same method that found the icon-sweep bugs; a style audit structurally cannot see any of this). `Plot` output was clipped on every reader-facing surface: `.ironpad-output-svg` (editor) sets `max-width:100%` + `height:auto` on the plot SVG, and the `.view-only-output-svg` rule beside it included the shared hover mixin and DROPPED those two declarations, so `/public`, `/shared`, `/mutable` and all three `/embed` routes hid **476px of an 812px chart at 390px — 59% of it, title included** — 266px (33%) at a 600px embed width, and 98px (12%) at iPad portrait, reachable only by a horizontal drag nested inside a vertical scroll. (Measured on prod at each width from the same `scrollWidth - clientWidth` metric. An earlier note, and commit `b0d9876`'s message, put "266px" against 390px; that figure was taken at 600px and carried over. The phone case is the worse one.) Both rules read correctly; only a rendered width tells the truth. The mixin is now `plot-svg` and OWNS the sizing, so the two paths cannot diverge that way again. Slider rows clipped at BOTH ends at once: `.ironpad-widget-label` carried `min-width: 200px`, turning a 200px *preferred* width into a floor and the row minimum into 395px, so the label lost its first word off the left while the value readout sat past the right border (`min-width: 0` lets the ellipsis that was already there do its job). Touch targets: ONE `ip-touch-target` mixin under `(pointer: coarse)`, included by BOTH shape mixins so anything built from either is covered without its call site remembering. The spec sweeps every visible `button`/`a[href]` rather than a named list, which is what turned up EIGHT offenders instead of the four already seen — and the first draft of that test named `.ironpad-theme-toggle-segment` believing it meant the header squares, so it silently measured the view-only Cached/Fresh pills instead (that shared class name was already a documented naming problem; it has now also produced a test that passed while checking the wrong element). The signed-in auth header needed MARKUP, not sizing: it was a `<span>` wrapping an `<a>` on the 0.6rem "Sign out" text alone, so the portrait was dead to the pointer, and a 44px label inside a 48px header would have shoved it out — the stack is now one `<a>`, mirroring the signed-out control it was already meant to match. `prefers-reduced-motion` is scoped to the CHROME with NO blanket `*` reset: that recipe reaches into author-supplied LiveView markup, and a reader who asked for less motion still came to watch a double pendulum. Five named animations and two decorative transforms, with the toggle-switch `translateX` deliberately kept because it encodes state rather than decoration. That block sits LAST in `main.scss` because a media query adds no specificity, so source order is the only thing deciding the winner and an earlier placement loses to every declaration below it while looking equally correct. `tests/e2e/responsive.spec.ts` covers all of it and GUARDS the emulation with `matchMedia` assertions, because `test.use({ reducedMotion })` silently did not reach the page and a preference test that runs without the preference is worse than no test. Em-dashes: 91 removed from the 24 demo notebooks' PROSE (markdown cells + title + description ONLY — code comments are exempt per `my-voice`, and the capture manifest is keyed on code source + shared context, so `.capture-manifest.json` came out byte-identical and nothing needed recapture), plus six user-visible UI strings in Rust; the 16 `blog` notebooks were already at zero, which is exactly `voice-lint`'s coverage boundary. `gen-blocks` gained a resume-arguments section (cells 12-13), placed so every runnable cell keeps its `id`, the manifest being keyed by id. PREVIOUSLY — Fly cold start was **8.24s** to first byte (0.19s warm) and **5.87s of it was one `rustc --version`**: `prewarm` was awaited before `bind`, and the rustup shim must demand-page `libLLVM.so` (199MB) + `librustc_driver` (152MB) before it can print a version string (40ms once warm). Two fixes. `prewarm` is no longer awaited — it is warm-up, not a prerequisite, and the statics behind it are `LazyLock`, so a request that beats it blocks on the lock exactly as it would with no prewarm at all. And the deploy image BAKES the answer: a Dockerfile `RUN` writes both raw `--version` outputs to `/app/toolchain-versions`, `IRONPAD_TOOLCHAIN_VERSIONS_FILE` points the server at it, and it is read back through the SAME parsers used on live output — the fingerprint is part of the compile cache key, so a one-byte difference would cold-start every cached blob (verified byte-identical against prod's live strings BEFORE shipping, in the real binary, not just in unit tests). Missing/unreadable/malformed falls back to probing; the image build fails loudly on a truncated file; the boot log carries `toolchain fingerprint source=baked|probed` so a deploy that lost the bake is visible at a glance. Measuring beat reading here: the same release binary covers that span in **23ms** locally with a warm page cache, which is what ruled out `generate_route_list` and the SurrealKV open (339ms). Fly `auto_stop_machines='suspend'` is NOT available as a lever — suspend is documented at **≤2GB** and this app needs 4GB so 4-way rustc and Enzyme fat-LTO do not OOM. Latent, not yet a problem: the SurrealKV WAL is one segment that has never rotated (25,424 batches, 2.2MB, replayed on every boot at ~37ms) and replayed on every boot. **CORRECTED 2026-08-11**: this note originally blamed `define_schema`'s 28 idempotent DEFINEs and the session sweep for writing on every boot. That is FALSE and was never measured. Probing WAL file size around each operation (with `upsert_user` as a positive control, +151 bytes) shows re-running the whole DDL against an existing schema appends **0 bytes**, and so does a `DELETE` matching no rows: SurrealDB elides both. Boot count is not the driver. The WAL grows from ordinary writes (draft autosaves, session renewals, share updates) and simply never shrinks, because no SSTable has been written and the memtable has not reached its flush threshold, so replay is O(every write ever made). Which write dominates has NOT been measured. Also: the agent-session panel closes on an outside click (`components/dismiss.rs`). The mechanism already existed for the toolbar dropdowns and carried the one comment worth not duplicating — it must read `composedPath()`, not `target.closest()`, because a menu item that toggles state detaches its own click target mid-dispatch — so it was lifted rather than copied. The selector names the WRAPPER, not the floating panel, so the trigger still reaches its own toggle instead of reading as a dismissal; `clear_if_open` guards the write because a dismissal callback runs on EVERY click in the document and Leptos `set` notifies whether or not the value moved. `session.spec.ts` asserts both directions (an inside click on Show/Hide must NOT close it) since only the negative half catches a naive implementation. PREVIOUSLY — Agent-session control is an icon-only square in the toolbar's right-hand group (menu/gear/close) with a corner guest-count badge; its wording lives in the tooltip + aria-label, which an icon-only button needs anyway (the svg is aria-hidden). That removes the reflow at the source and the toolbar is ONE row again — the second row stopped the button shoving Push sideways but spent a whole row on a control most sessions never touch. `session.spec.ts` asserts the invariant (button width before connecting an agent == width after the badge appears), not the old layout. Header theme toggle drops to 24px squares via `--compact`: `.ironpad-theme-toggle` is SHARED with the Cached/Fresh cache toggle in the view-only toolbar, which sits beside Run All and must stay 34px (that two unrelated controls share a "theme toggle" class is a real naming problem, deliberately left). The first version of that shrink was INERT — `height: 24px` against the shared pill rule's `min-height: 34px`, and min-height wins; 118 green specs missed it because none measured it, so the reset lives in `ip-icon-button-shape` (any square from that mixin is beatable the same way) and embed.spec.ts now measures BOTH halves of the split. PREVIOUSLY — Icon-sweep completion (found by driving the DEPLOYED site, not by reading the diff): `PRIVATE` and `PUBLIC` both point at one Lucide diamond and the filled-vs-outline reading (`◆` vs `◇`) lives only in `.ironpad-notebook-badge.private svg { fill: currentColor }` — that rule was never written, so the two badges rendered byte-identical and the home page could not tell its storage classes apart (icons.rs claimed a `--filled` class preserved it; the class did not exist). An audit of all 55 roles found the only two with no call site, both from mapping by GLYPH SHAPE rather than by affordance: `ADD` claimed `⊞`, which was really "⊞ Export HTML" (already `EXPORT`), while the actual add buttons kept an ASCII `+`; `REFRESH` claimed `⟳`, whose three uses were already `PUBLISHED`/`RERUN`/`PENDING` (deleted). `glyph-check` gained an ASCII arm — a character does an icon's job as well as an emoji does, and the U+2000 floor was blind to it; a LABEL is whitespace-then-a-word with no code punctuation, which distinguishes it from generated Rust/HTML string literals that also open on punctuation. Renaming those labels broke 52 specs through `createNotebook`'s literal "+ New Notebook" filter, so add-cell selectors are now `ADD_CODE`/`ADD_MARKDOWN` in `tests/e2e/helpers/session.ts` (class, not prose — and `hasText: "Code"` would now also match the cell's Code/Cargo.toml tab). PREVIOUSLY — UI consistency pass: ONE button geometry (`ip-button-shape`/`ip-icon-button-shape` mixins in `style/main.scss`, two size tiers as mixin defaults) replacing nine hand-authored paddings/fonts across Run All, session, Push, Fork, Embed, Edit, Restore, the toolbar icon squares, and both segmented controls; the notebook toolbar split into two rows (Run All + Push on top, session below — its label swings from "Start Agent Session" to "2 agents" and shoved Push sideways inline) with the draft indicator moved AFTER Push (its show/hide reflow slid the button out from under the cursor); auth header signed-in state is now the same portrait-over-label stack as signed-out (avatar over "Sign out", handle in the tooltip, shared `AuthSilhouette` fallback when a user has no GitHub picture); "Fork to Private" → "Fork" everywhere (the `fork_label` prop deleted — all four callers passed one string); "View as Reader" → "View Published" and the bottom-left toggle retitled "Preview" (they show PUBLISHED vs YOUR DRAFT and read as duplicates until the labels say so; both mode segments gained aria-labels, which is also what e2e now selects); Unpublish gained an "Unpublishing…" progress toast; `--ip-bg`/`--ip-bg-secondary`/`--ip-text` were NEVER DEFINED, so the version-history panel rendered transparent over the notebook — fixed, plus `tools/css-vars-check.py` in `ci` (an undefined var() with no fallback drops the whole declaration silently, and neither clippy nor Playwright could see it); `glyph-check`'s box-drawing exemption removed (it was hiding three live `╳` affordances and only ever fired on false negatives) with `icons::DELETE`/`icons::SESSION` added. PREVIOUSLY — Second-pass fanout fixes (post-remediation): disposal-safe blame helpers (`try_with_untracked` — the W1 unification had turned a disposal-safe write into a panicking read; navigating away from an autorunning viewer mid-compile aborted the whole wasm app), `NotebookState::scrub_deleted_cell` shared by UI delete AND agent CellDelete (ghost queue fronts stalled Run All) + `merged_run_queue` never pins a front absent from cells, `Event::RunCancelled` (inside the unshipped v6 bump) reports user-terminate drops and daemon inference is grace-held (`RunSignal` Terminal/Inferred/OursAlive, `apply_signal`, `INFERENCE_GRACE_MS` 1.5s — a stale-cache guess defers to the browser's report or the target's own activity), capture zero-capture guard (0 captured against a snapshot-bearing file = keep + exit 1; `--allow-empty` overrides; FAILED arm exits nonzero), manifest re-serialized in `recordManifest` key order, `CellRunCtx::fail_and_report` folds the three drop-report loops, `tests/e2e/helpers/menu.ts` (MENU/menuClick, re-exported from mutable.ts) adopted by ALL specs + `shareMutable` replaces the two remaining weak inline copies; Fanout-review remediation (post-v0.18.0, three waves): W1 cascade cluster — queue/blame bookkeeping shared via `run_flow` (`fail_in_queue`/`clear_own_blame`/`clear_blame_held_by`; editor and viewer delegate, sticky-⛔-after-successful-rerun fixed, `delete_cell_fn` cleans both blame sides), cascade MERGES into an in-flight queue (`merged_run_queue`: pinned front + notebook-ordered union; replacing dropped queued independents), `Event::CellBlocked` (PROTOCOL_VERSION 6) makes browser queue-drops authoritative for `cells.run` (inference kept as pre-v6 fallback) and `CellDeleted` for the target is terminal (`cell_deleted`); W2 capture tooling — seeded scratch copies stripped of prior `saved_output` (preserve-on-no-capture can't re-ship stale snapshots under fresh hashes; set/delete write-back on every path), freshness manifest widened to `{cells: sha256(source NUL cargo_toml), shared: sha256(shared context)}` in BOTH tools (shared-cell/shared-source/cargo_toml edits now flag stale), zero-runnable notebooks get manifest entries (markdown-only notebook no longer bricks ci); W3 sweep — `private_share_readable` is the one private-gate predicate (notebook + manifest cores; manifest signed-in arms unit-tested), whitespace-tolerant `uses_gen_blocks` (`gen{` no longer dead-ends on an unreachable E0658 gate), `run_flow::sleep_ms` the one sleeper, `tests/e2e/helpers/mutable.ts` (MENU/menuClick/shareMutable shared by four specs), `ShareAccessSection` → `share_access.rs`; OG/oEmbed cache TTL after flip-to-private deliberately left (review decision); Run-flow unification (PRD-0059): one blob-acquisition/execute engine (`components/run_flow.rs`, snapshot -> local probe -> compile with transport retries) consumed by editor + viewer, editor gains retries; Dependency-aware cascade + continue-past-failures (PRD-0060): `ironpad_common::cell_deps`, referenced-slot-only scaffold bindings, key normalization inside the hash, edition-2024 cells (the v0.17.0 gen_blocks gate was inert on 2021 — never deployed), `cell_main` trampoline shielding user tokens from wasm-bindgen's syn, CACHE_EPOCH 9, daemon prerequisite_failed now dependency-aware, borrows/dynosaur finally capture outputs; gen-blocks public notebook (46th; movable-vs-static coroutine story, E0626 teaching cell); capture-outputs cargo-make task + sha256 freshness manifest checked in ci; Private mutable shares (PRD-0061): private flag + READ grants by GitHub handle, one access core gating reader/embed/manifest/OG/oEmbed, Access UI in the metadata panel; /local version history (PRD-0058, unreleased): IndexedDB `history` ring (DB_VERSION 6, 5-min buckets, cap 30, dies with the notebook), History panel + confirm-gated undoable Restore; /embed/mutable + oEmbed for published notebooks (PRD-0057); gen_blocks feature gate; Persisted cell outputs (PRD-0056, unreleased): `IronpadCell.saved_output` + `embed_saved_outputs` (preserve-on-no-capture), capture at Share/Push-durable/Download, `PanelMode::Snapshot` with static Simulation/LiveView arms, `.view-only-saved-badge` serialized-output affordance, `PROTOCOL_VERSION` 5; SOC refactors (PRD-0055, unreleased): `notebook_editor/pipeline.rs` (CellItem's compile/execute pipeline + live-check dispatch behind a `CellRunCtx`; cell_item.rs 2,073 → 1,480), executor JS split (`executor-gpu.js` WebGPU runtime, `executor-glue.js` env table + glue rewriting, `executor-core.js` 1,480 → 1,070 keeps class/ABI/sim-bus; loaders updated in both contexts, env sync test retargeted to the glue file), `main.rs` split into `cache_valve.rs`/`http_policy.rs`/`otel.rs` bin modules (main.rs 845 → ~330); Fanout-review remediation, three waves (v0.16.2): W1 — reactive route params on all five notebook pages (same-route navigation used to keep rendering/EDITING the old notebook; markdown cross-links between public notebooks hit it), `notebook_editor/sharing.rs` extraction with one flush-before-serialize helper (fixes Unpublish losing the last ~1s of typing, Push promoting a stale draft behind a success toast when the durable save failed, Download .ironpad silently no-op in the mutable editor; orphaned exportNotebook seam deleted at all three layers), draft autosaves counted toward `MAX_TOTAL_MUTABLE_BYTES`; W2 — `run_group_with_timeout` (build/check/wasm-opt/wasm-bindgen share one process-group kill discipline; orphaned rustc children died with cargo), sliding session cookie actually slides (`session_user` reports renewals, `current_user` re-issues Max-Age; cookie mint/clear + TTL single-sourced in `ironpad_app::auth`), expired sessions disconnect their guests (`WsState::sweep_expired_sessions`), `WidgetSink` carries runnability (button widgets stalled the view-only queue on shared cells), `cells.run` terminal on runtime prerequisite failures, session panel teaches the real CLI command, executor-worker onerror terminates-then-respawns with a rapid-failure cap (main-thread fallback, e2e-tested via synthetic ErrorEvents), one generated env host-import table in executor-core.js (+ sync test against ironpad-cell's extern blocks), toolchain-pin sync test, `assemble_cell_inputs`/`unexecuted_upstream`/blob-cache probe policy shared editor↔viewer, `ironpad_common::notebook_ops` shared model↔daemon; W3 — viewer per-cell Run cascades unexecuted upstream cells, dotted `[dependencies.X]` subtables parsed (merge/rayon-detection/ironpad-cell filter), `PROTOCOL_VERSION` 4 (`ErrorCode::Unknown`, explicit-null cargo_toml clear survives the wire), daemon init snapshot cached on the recv path (event-loss race), GPU readbacks claim distinct panels, `safe_redirect_path` rejects control chars, promote records byte (not char) sizes, wasm-opt temp cleanup on all paths, duplicated defaults single-sourced; Draft/published mutable shares (PRD-0054): server-authoritative drafts (`draft_json` slot, debounced epoch-coalesced autosave, promote-on-Push with `Ok(false)` for clean, Discard Draft, View as Reader), unified `/mutable/{id}` (owner editor swap on hydrate via `NotebookEditor` + `server_draft` prop; SSR always the reader), local mutable store deleted (storage.js DB_VERSION 5, three-source home listing), Push toolbar button (`Published ✓`/`⬆ Push`), unpublish saves-local-then-deletes; header sign-in placement fix (auth surface moved from the home-hidden status bar to the header far right); Accounts (PRD-0053): GitHub OAuth sign-in (`ironpad-server/src/auth.rs`, nest_service /auth), embedded SurrealDB (`ironpad-app/src/db.rs`, SurrealKV at {data_dir}/ironpad.db, user/session/mutable_share/rbac_grant tables, OWNER-only grants shaped for EDIT/READ later), sessions in an HttpOnly/Secure/SameSite=Lax cookie stored blake3-hashed with sliding 30-day expiry, env-gated /auth/test-login for e2e (IRONPAD_TEST_AUTH; absent otherwise, asserted by test), mutable shares rewritten onto the session's OWNER grant with content+manifest in the DB (two-key mechanism deleted: user key, notebook key, rebind form, derive_key, subtle), reader-page owner attribution + clone-to-local Edit, footer sign-in/avatar, storage.js DB_VERSION 4 in-place migration, get_auth_info + MutableNotebookResponse wire types, OG mutable cards via axum Extension(Db); CLI `notebook update` (PRD-0051 gap-closer): agents edit notebook-level shared source/Cargo.toml, title, description, tags via a tri-state `NotebookMetaPatch` (`--clear-*` = explicit-null clear; daemon deserializes the patch verbatim so the CLI tracks the protocol; PROTOCOL_VERSION unchanged); Progress toasts + action feedback (v0.14.2): Info intent, `Toaster::toast_after_reload` (sessionStorage across Pull's reload), Pull's Up-to-Date short-circuit, rebind/fork toasts; Mutable-share author round-trip (PRD-0049 follow-up): binding-aware `/mutable` reader ("✎ Edit" toolbar shortcut + menu swap on the authoring device, rebind form always verifies), author-only divergence banner backed by `IronpadNotebook::content_matches` (ignores `updated_at`), editor Pull Latest (confirm-gated local overwrite + reload) and View Published menu items, published URL + copy in the metadata panel, `ViewOnlyNotebook` `controls` slot prop; Agent-triggered execution + build admission control (PRD-0052): `ironpad cells run` (CellRun mutation intercepted pre-model, run-queue dispatch, session-gated execution events, daemon broadcast wait keyed by cell_id, PROTOCOL_VERSION 3), `BuildAdmission` (global cargo cap w/ bounded queue for compiles + Skipped-degrading checks, per-IP token bucket charged only on cache MISSES); Pipeline trace instrumentation: `#[tracing::instrument]` spans (always `skip_all` — payloads carry user source) across compile/check/share/mutable/OG request paths, root request spans named by route template via `otel.name` with query strings excluded (the `/ws/connect` token lived in the exported URI field), lock-wait and `spawn_blocking` rasterize spans, outcome fields (`cache`, `status`, `hit`) recorded on spans; span taxonomy table in DEVELOPMENT.md Observability; Notebook metadata + oEmbed (PRD-0051): `description`/`tags`/`og_image` are editable from a panel below the cell list (`pages/notebook_editor/metadata_panel.rs`), new `og_image_width`/`og_image_height` gated by `og_image_dimensions()` so an override stops advertising 1200x630, `NotebookMetaPatch` flattened into BOTH `Mutation::NotebookUpdateMeta` and `Event::NotebookMetaUpdated` (wire format unchanged; `apply_to` replaces the CLI daemon's hand-written mirror; `PROTOCOL_VERSION` 2), `explicit_null_is_a_clear` fixes doubled-option clears that serde silently decoded as "unchanged", oEmbed provider at `/oembed` + discovery links, CSP (`object-src`/`base-uri`/`form-action`; deliberately no `script-src` or `frame-ancestors`), public-notebook scan cached per process, single-flight card rendering, `absolute_url` wired; social-preview security patch: stored XSS via the raw `<title>` splice, quadratic `ellipsize`/`expand_tabs`, a card cache that never evicted, and soft-404s on the async routes (v0.13.1); Social previews (PRD-0050): per-page Open Graph/Twitter metadata behind `SsrMode::Async`, server-generated 1200x630 preview cards at `/og/*` via resvg with embedded fonts, `og_image` override, robots.txt + sitemap.xml, `IRONPAD_PUBLIC_URL`; Coroutine cells (`#[coroutine]` feature gate, fourth cell special case) and the `pin-coroutines` notebook; AI-authorship disclosure banners on all 14 `blog` notebooks; voice pass removing meta-discourse, self-certification, and contrast-definition across them (`voice-lint` skill in dotagent is the gate for public prose); AI-authorship disclosure banners on the 14 `blog` notebooks (`> *AI generated.*`; cannon reads `human edited`), meta-discourse/self-certification voice pass across all of them, ammonia 4.1.4 for RUSTSEC-2026-0213, Share Immutable button label (v0.12.17); Mutable shares (PRD-0049): author-updatable /mutable/{id} conversion, two-key (user + notebook) push auth hashed with domain-separated blake3, reader-page rebind, footer user key, home Published group (v0.12.16); Blog-notebook voice pass + cannon rewrite with real Enzyme IR/WAT excerpts, Prism wasm/llvm grammars (v0.12.15); Canonical routes /local, /public, /shared with legacy redirects (PRD-0048); notebook menu + close in view mode; Static blob delivery (PRD-0047): share-time blob snapshots + immutable /share-blobs route + client IndexedDB blob cache, cache-key recipe moved to ironpad-common (v0.12.14); Thaw removed: native UI primitives + owned toaster (v0.12.13); session teardown on page disposal + disposal-read guards (v0.12.12); view mode renders the public notebook renderer + code-wide papercut sweep (v0.12.11); per-cell collapse defaults (v0.12.10); live check-on-type + completions (PRD-0045); shared cells; unified toolchains
