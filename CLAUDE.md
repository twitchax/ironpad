...
**Mutable shares (PRD-0049; accounts-backed since PRD-0053)** are the author-updatable storage class. `Share Mutable` *converts* a private notebook into a server-backed one at `/mutable/{id}` (server-minted 16-hex id): anyone with the link reads it; the owner overwrites it with an explicit **Push**. Ownership is the signed-in account's OWNER grant — create requires a session, push/delete require the grant, and the PRD-0049 two-key mechanism (user key, notebook key, rebind form, `derive_key`/`subtle`) is deleted. Share content (notebook JSON + manifest) lives in the DB record, transactional with its grant; blobs still reuse the immutable `shares/blobs/` store (served by `/share-blobs/`), and mutable shares REPLACE their embedded manifest each push (immutable shares MERGE). Server fns: `create_mutable_share(notebook_json, tags)`, `push_mutable(id, notebook_json, tags)`, `get_mutable_notebook(id)` → `MutableNotebookResponse {notebook, owner, is_owner}` (attribution + caller ownership resolved server-side), `get_mutable_manifest`, `delete_mutable_share(id)` (unpublish), `list_mutable_shares()` (by session; anonymous → empty). Client: the `mutable` IndexedDB store keeps the share_id→working-copy binding but no keys (storage.js DB_VERSION 4 migrates in place); `getNotebook`/`saveNotebook` route transparently. The reader page shows "Published by @login" attribution for everyone; the owner gets "✎ Edit" — a link into `/local/{uuid}` when a working copy exists, a **clone-to-local** button otherwise (the rebind replacement: sign in on any device and Edit clones the published copy). The divergence banner (`IronpadNotebook::content_matches`, ignores `updated_at`) and the editor's Share Mutable↔Push / Delete↔Unpublish / Pull Latest / View Published behavior are unchanged. Only the id→content resolve is mutable (no-cache by default policy).
...
- `/mutable/{id}` — MutableNotebookPage (read-only reader + owner attribution + clone-to-local Edit; author-updatable via Push; PRD-0049/0053)
...
**Agent-triggered execution (PRD-0052)**: `ironpad cells run <cell_id>` executes a cell in the hosting browser and blocks until its terminal event. `Mutation::CellRun` rides the mutation envelope (write-permission gate) but is intercepted in `session/connection.rs` BEFORE `model.apply` — execution is an editor action, not a state mutation — and dispatched to the same `run_all_queue` Run All uses, so prerequisites cascade. The browser emits `CellCompiling`/`CellCompiled{success}`/`CellExecuted{success}` from the compile flow (session-gated via `NotebookModel::emit_event`; nothing is emitted without a live session); the CLI daemon fans events out on a broadcast channel and `cells.run` correlates by `cell_id`, terminal on `CellExecuted` (ours), `CellCompiled{success:false}` (ours = `compile_error`, any other = `prerequisite_failed` since the queue clears on compile failure). `PROTOCOL_VERSION` 3.
...
Current server functions: `compile_cell`, `check_cell` (live check-on-type, PRD-0045), `list_public_notebooks`, `get_public_notebook`, `share_notebook`, `get_shared_notebook`, `get_shared_manifest` (blob-snapshot sidecar, PRD-0047), `get_toolchain_fingerprint` (client cache keys, PRD-0047), the mutable-share set (PRD-0049/0053, session-gated writes): `create_mutable_share`, `push_mutable`, `get_mutable_notebook`, `get_mutable_manifest`, `delete_mutable_share`, `list_mutable_shares`, and `get_auth_info` (footer auth surface, PRD-0053).
