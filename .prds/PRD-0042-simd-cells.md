...
- 2026-07-11 (v0.7.1): prod hotfix — the deploy image's default toolchain is stable, so the injected portable_simd gate failed on prod ("may not be used on the stable release channel") while every local gate ran on nightly. SIMD-only cells now pin the autodiff nightly; unit tests cover selection and priority. Verified live: autovec f32 40ms refused / i32 7ms free, explicit 29 -> 8 -> 5ms (5.8x), Mandelbrot 165 -> 54ms with the image rendered.
- 2026-07-11 (v0.7.2): root-caused the toolchain divergence instead of patching around it. ALL cell builds now pin explicitly: CELL_TOOLCHAIN (nightly-2026-06-01, one ungated const shared by the compiler, the cache fingerprint, and the footer) for everything except rayon cells (atomics pin). The first honest docker-uat run this enabled found three stacked image faults: compose testing a months-stale image tag, both warmups dead since PRD-0012's workspace.lints adoption (silenced by '|| true'), and an entrypoint that crash-looped fresh volumes when the warmup artifacts were missing. All fixed; warmup failures now fail the image build loudly.
