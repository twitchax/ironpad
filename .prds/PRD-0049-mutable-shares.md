...
- id: T-004
  title: "Editor: convert, push, unpublish (staleness deferred)"
  priority: 1
  status: done
  notes: "DONE: hamburger menu swaps Share Mutable→Push Update and Delete→Unpublish on binding state. Share Mutable converts (create on server, move notebook from private store to mutable store under the minted id); Push overwrites + re-snapshots via the stored edit key; Unpublish deletes server-side and converts back to private. Home lists mutable-backed notebooks in a Published group. DEFERRED to a follow-up: the open-time staleness check (compare against the server copy, offer to pull when it's newer). Current behavior is last-write-wins on Push — a proper guard needs a sync-baseline (store the server's pushed_at locally on create/push/rebind and compare on open), which content comparison alone can't do because local unpushed edits are indistinguishable from a remote change. Narrow multi-device-same-author race; not yet guarded."
- id: T-005
