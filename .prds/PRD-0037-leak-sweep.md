...
## 2026-07-03 — Complete (7/7)
All closure/timer/listener leaks swept on branch `fix/prd-0037-leak-sweep`, each wasm-target-clippy clean + full gate green:
- T-001 StatusBar interval, T-002 Monaco on_change, T-003 document keydown + outside-click listeners, T-004 markdown Escape keybinding, T-006 CopyButton timeout, T-007 cell debounce timers + editor_handles + run/advance Monaco actions: all moved off closure.forget() to arena-held storage (StoredValue::new_local) dropped on dispose, with interval/timeout/listener handles cleared in on_cleanup.
- T-005 broke the rAF closure Rc cycle (cb ↔ Closure) in all three animation/simulation/live-view loops by clearing the arena-held closure in on_cleanup.
- **Deferred**: the rAF loops still reschedule while paused (a CPU optimization, not a leak — a control-flow redesign for a focused pass); and a few Effect-scoped one-shot timeouts (save-status reset, focus delay, SortableJS on_end) that need per-handle clear/destroy lifecycles.
